...
export async function renderRegister(container) {
  ...
  const html = `
    <div class="min-h-screen bg-slate-50 flex flex-col justify-center py-12 px-6 lg:px-8">
      <div class="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <h2 class="headline text-3xl font-black text-slate-800 tracking-tight">Daftar Member</h2>
        ...
        <div class="bg-white py-8 px-8 shadow-2xl shadow-slate-200/50 rounded-[2.5rem] border border-slate-100">
          <form id="register-form" class="space-y-6">
            ...
            <div>
              ...
              <p class="mt-2 text-[10px] text-slate-400 font-medium">*Nomor ini akan menjadi ID akun Anda.</p>
            </div>
          ...
          <p class="mt-8 text-center text-xs text-slate-400 font-medium">
            Sudah punya akun? <a href="#/home" class="text-primary font-bold hover:underline">Masuk di sini</a>
          </p>
        </div>
      </div>
    </div>
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (existingUser) {
        // Simple auto-login if exists (as per user request: no-auth)
      ...
      console.error(err);
      errorMsg.textContent = err.message || 'Terjadi kesalahan saat mendaftar.';
      errorDiv.classList.remove('hidden');
      submitBtn.disabled = false;
      submitBtn.innerHTML = '<span>Daftar Sekarang</span><span class="material-symbols-outlined">rocket_launch</span>';
    }
  });
}
